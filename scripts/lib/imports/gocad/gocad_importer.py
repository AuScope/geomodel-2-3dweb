...
class GocadImporter():
    ...
    def process_gocad(self, src_dir, filename_str, file_lines):
        ...
        if self.__is_vo or self.__is_sg:
            ret_val = self.__read_volume_binary_files()
    ...
    def __read_volume_binary_files(self):
                    ...
                    for z_val in range(self.vol_sz[2]):
                        for y_val in range(self.vol_sz[1]):
                            for x_val in range(self.vol_sz[0]):
                                ...
                                x_coord, y_coord, z_coord = self.__calc_vo_xyz(x_val, y_val, z_val, mult)
                                # If numeric VOXET
        ...
        if not self.SKIP_FLAGS_FILE:
            ...
            if self.flags_file != '' and self.__is_vo:
                self.__read_region_flags_file(self.flags_array_length,
                                              self.flags_file,
                                              self.flags_bit_size,
                                              self.flags_offset)

            # SGRID also has a flags file, but uses different markers
            elif self.region_flags_file != '' and self.__is_sg:
                self.__read_region_flags_file(self.region_flags_array_length,
    ...
    def __calc_vo_xyz(self, x_idx, y_idx, z_idx, mult):
    ...
    def __read_region_flags_file(self, flags_array_len, flags_file,
        ...
        if flags_array_len != self.vol_sz[0]*self.vol_sz[1]*self.vol_sz[2]:
            ...
            self.logger.debug("__read_region_flags_file() return False")
            return False
